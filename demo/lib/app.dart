...
class MyAppState extends State<MyApp> with SingleTickerProviderStateMixin {
  ...
  void _showPrintedToast(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Imprimiendo documento...'),
      ),
    );
  }
  ...
  void _showSharedToast(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Descargando archivo pdf...'),
      ),
    );
  }
  ...
  Future<void> _saveAsFile(
    ...
    final file = File(appDocPath + '/' + 'curriculum_vitae.pdf');
    print('Guardando documento ${file.path} ...');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('CV en Flutter'),
        bottom: TabBar(
  ...
  void _showSources() {
    ul.launchUrl(
      Uri.parse(
        'https://github.com/Maria-Jose-11/cv_en_flutter.git',
      ),
    );
  }
